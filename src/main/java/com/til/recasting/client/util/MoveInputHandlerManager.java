...
import com.til.recasting.client.register.key.LockClientKeyRegister;
import com.til.recasting.client.register.key.SummonedSwordClientKeyRegister;
...
@OnlyIn(Dist.CLIENT)
public class MoveInputHandlerManager implements IWorldComponent {
    @CapabilityInject(IInputState.class)
    public static Capability<IInputState> INPUT_STATE = null;

    @VoluntarilyAssignment
    protected SummonedSwordClientKeyRegister summonedSwordClientKeyRegister;

    @VoluntarilyAssignment
    protected LockClientKeyRegister lockClientKeyRegister;
    ...
    public void onPlayerPostTick(TickEvent.PlayerTickEvent event) {
        ...
        if (lockClientKeyRegister.getKeyMapping().isKeyDown()) {
            commands.add(InputCommand.SNEAK);
        }
...
*/
        ...
        if (summonedSwordClientKeyRegister.getKeyMapping().isKeyDown()) {
            commands.add(InputCommand.M_DOWN);
        }
        ...
        EnumSet<InputCommand> old = player.getCapability(INPUT_STATE).map(IInputState::getCommands).orElseGet(() -> EnumSet.noneOf(InputCommand.class));
        ...
        */
        ...
        if (doCopy && old.contains(InputCommand.SAVE_TOOLBAR) && !commands.contains(InputCommand.SAVE_TOOLBAR)) {
            ...
            if (KeyModifier.SHIFT.isActive(KeyConflictContext.UNIVERSAL)) {
                ...
                if (stack.getCount() != 1) ret.addProperty("count", stack.getCount());

                CompoundNBT tag = new CompoundNBT();
                stack.write(tag);

                CompoundNBT nbt = stack.getOrCreateTag().copy();
                if (tag.contains("ForgeCaps")) nbt.put("ForgeCaps", tag.get("ForgeCaps"));
                ...
                if (KeyModifier.CONTROL.isActive(KeyConflictContext.UNIVERSAL)) {
                    ...
                    {
                        NBTHelper.NBTCoupler nbtc = NBTHelper.getNBTCoupler(checktarget).getChild("ForgeCaps").getChild("slashblade:bladestate").getChild("State");
                ...
                if (stack.getTag() != null && element != null) ret.add("nbt", element);
